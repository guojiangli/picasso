...
func RestrictParallel(restrict int, fns ...func()) func() {
	channel := make(chan struct{}, restrict)
	return func() {
		var wg sync.WaitGroup
		for _, fn := range fns {
			wg.Add(1)
	...
	go func() {
		defer func() {
			if err := recover(); err != nil {
				log.Error("recover", kstring.KVInterface("err", err), kstring.KVInterface("line", fmt.Sprintf("%s:%d", file, line)))
			}
		}()
		// 忽略返回值, goroutine执行的返回值通常都会忽略掉
		_, err := inj.Invoke(fn)
		if err != nil {
			log.Error("inject", kstring.KVInterface("err", err), kstring.KVInterface("line", fmt.Sprintf("%s:%d", file, line)))
			return
		}
	}()
}
